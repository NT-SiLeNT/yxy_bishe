			...
			<uni-card v-for="(item, index) in listData" :key="index" @tBtnHandle="tBtnHandle(item.serno)" :isBgImg="true">
				<template>
					...
					<view class="inline-btn inline-btn-right">
						...
						<button class="mc-min-btn-primary" :disabled="item.cus_status == '20'" @click="changeCust({ cus_id: item.cus_id, cus_status: item.cus_status })">
							修改
						</button>
					</view>
				</template>
	...
	methods: {
		...
		changeCust(item) {
			...
			uni.setStorage({
				...
				success: function() {
					_this.toLink('/pages/customerMg/customerAdd?isChange=true&isquery=true&custStatus=' + item.cus_status + '&cusId=' + item.cus_id);
				}
			});
		},
		queryCust(item) {
			...
			uni.setStorage({
				...
				data: _this.$androidenv ? document.getElementsByTagName('body')[0].scrollTop : document.getElementsByTagName('html')[0].scrollTop,
				success: function() {
					_this.toLink('/pages/customerMg/customerAdd?isQueryDetail=true&isquery=true&custStatus=' + item.cus_status + '&cusId=' + item.cus_id);
				}
			});
		},
