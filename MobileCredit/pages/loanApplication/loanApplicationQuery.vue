			...
			>
				<template>
					...
					<view class="inline-btn inline-btn-right">
						<button :disabled="disabledOperBtn(item.biz_type_sub)" class="mc-min-btn-primary" @click="queryCust(item)">查看</button>
					</view>
				</template>
			</uni-card>
		</block>
		...
		<uni-drawer :visible="showRight" mode="right" @close="closeDrawer('right')" class="mcdrawer">
			...
			<view class="mcform">
				...
				<!-- <view class="uni-form-item uni-column onerow bottomGap">
					...
					<input v-model="search.prd_name" type="text" class="uni-input" value="" placeholder="请输入产品名称" />
				</view> -->
				<view class="uni-form-item uni-column onerow bottomGap">
				...
				<view class="uni-form-item uni-column">
					...
					<view class="uni-input linebtn">
						<button
							@click="changeSelect('approve_status', '997')"
							type=""
							class="mc-min-btn-primaryplan"
							value="997"
							:class="{ checked: search.approve_status == '997' ? true : false }"
						>
							通过
						</button>
						<button
							@click="changeSelect('approve_status', '111')"
							type=""
							class="mc-min-btn-primaryplan"
							value="111"
							:class="{ checked: search.approve_status == '111' ? true : false }"
						>
							审批中
						</button>
						<button
							@click="changeSelect('approve_status', '990')"
							type=""
							class="mc-min-btn-primaryplan"
							value="990"
							:class="{ checked: search.approve_status == '990' ? true : false }"
						>
							撤销
						</button>
						<button
							@click="changeSelect('approve_status', '998')"
							type=""
							class="mc-min-btn-primaryplan"
							value="998"
							:class="{ checked: search.approve_status == '998' ? true : false }"
						>
							否决(不同意)
						</button>
					</view>
	...
	methods: {
			...
			this.$request
				.post({
					...
					data: {
						...
						approve_status: this.search.approve_status !== '' ? this.search.approve_status : '997', //	审批状态
						apply_date: '', //申请日期
		...
		queryCust(item) {
			...
			uni.setStorage({
				...
				success: function() {
					_this.toLink('/pages/loanApplication/loanApplication?isQueryDetail=true&serno=' + item.serno + '&apply_date=' + item.apply_date);
				}
			});
		},
		...
		resetSearch(btn) {
			...
			// this.search.prd_name = '';
			this.search.cus_id = '';
			this.search.serno = ''; //客户类型
			this.search.approve_status = '997'; //客户状态
		},
		show(e) {
			this.showRight = true;
		},
