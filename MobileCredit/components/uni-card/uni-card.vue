<template>
	<view
		:class="{
			...
			'uni-card--bgimg': isBgImg === true || isBgImg === 'true'
		}"
		class="uni-card"
		@click="onClick"
	>
		<view v-if="mode === 'style'" class="uni-card__thumbnailimage">
			<view class="uni-card__thumbnailimage-box"><image class="uni-card__thumbnailimage-image" :src="thumbnail" mode="aspectFill" /></view>
		...
		<view v-if="mode === 'title'" class="uni-card__title">
			<view class="uni-card__title-header"><image class="uni-card__title-header-image" :src="thumbnail" mode="scaleToFill" /></view>
		...
		<view v-if="mode === 'basic' && title" class="uni-card__header">
			...
			<text class="uni-card__header-title-text leftw75" style="width: 75%;">
				{{ title }}
				<span v-if="title1" style="color: #939599;margin: 0 3px;">|</span>
				{{ title1 }}
			</text>
			<text v-if="titleBtn" class="uni-card__header-title-text right" :class="{'disabled':!titleBtncanUse}">
				<span @click.stop="(!titleBtncanUse?'':titleBtnClick())">{{ titleBtn }}</span>
			</text>
...
.uni-card {
	...
	display: flex;
	flex: 1;
	box-shadow: 0 2px 10px 0 rgba(48, 49, 51, 0.2);
	/* #endif */
	margin: 12px;
...
.uni-card--bgimg {
	   background: radial-gradient(circle at top right, #e4ecfa 7%, #ffffff 38%)
}
...
.uni-card__header-title-text.leftw75{
	width: 74%;
	line-height: 1.5;
	white-space: initial;
	overflow: inherit;
...
.uni-card__content {
	color: #333;
	box-shadow: 0px -1px 0px 0px #d5e1f7;
}
